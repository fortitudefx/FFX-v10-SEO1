...
// Articles per weekday cron run (1/weekday = 5/week). Overridable via env.
export function keywordsPerRun(env) {
  const n = parseInt((env && env.KEYWORDS_PER_RUN) || '1', 10);
...
// ── HEAD-TOPIC NORMALISATION ────────────────────────────────────────────────
// The `canonical` field is hand-assigned and turned out to be INCONSISTENT: some
// variants were folded onto the head term ("what is a liquidity sweep" →
// "liquidity sweep") while others kept their own ("what is liquidity sweep" →
// "what is liquidity sweep", "how to trade fair value gaps" → itself). Every
// extra canonical for one real topic = one extra near-duplicate article, which is
// how "how to trade fair value gap" AND "how to trade fair value gaps" both went
// live on 2026-07-24, and how 8 canonicals ended up covering 3 real topics.
//
// Normalising HERE rather than only fixing the data is deliberate: it repairs the
// already-seeded live map with no risky re-seed (a forced re-seed resets every
// status and would re-generate every published article), and it keeps holding as
// new "(discovered)" rows arrive with their own inconsistent canonicals.
// Pure string function — same input always yields the same topic.
const TOPIC_PREFIXES = [
  'what is a ', 'what is an ', 'what is ', 'what does a ', 'what does ',
  'how to trade ', 'how to spot a ', 'how to spot ', 'how to identify a ',
  'how to identify ', 'how to read ', 'how to ',
  'buy side ', 'buy-side ', 'sell side ', 'sell-side ', 'bullish ', 'bearish ',
];
const TOPIC_SUFFIXES = [
  ' look like', ' in trading', ' explained', ' definition', ' examples',
  ' example', ' meaning', ' strategies', ' strategy', ' indicator', ' entry',
  ' times', ' time', ' pdf', ' trading', ' forex',
];

export function headTopic(value) {
  let s = String(value || '').toLowerCase().trim().replace(/[^a-z0-9\s-]/g, ' ').replace(/\s+/g, ' ');
  // Strip qualifiers repeatedly — "how to spot a bullish liquidity sweep" needs
  // more than one pass to reach "liquidity sweep".
  let changed = true;
  while (changed) {
    changed = false;
    ...
    for (const suf of TOPIC_SUFFIXES) {
      if (s.endsWith(suf) && s.length > suf.length) { s = s.slice(0, -suf.length); changed = true; }
    }
    s = s.trim();
  }
  // Singularise the trailing noun so "gaps"/"blocks"/"concepts" fold onto their
  // singular head. Guard "ss" (process) and "us"/"is" endings.
  if (/[a-z]{3,}s$/.test(s) && !/(ss|us|is)$/.test(s)) s = s.slice(0, -1);
  return s.trim();
}

// The topic a row occupies. Prefers the curated canonical, falls back to the
// keyword, and normalises either way.
export function rowTopic(row) {
  return headTopic((row && row.canonical) || (row && row.keyword) || '');
}

// { headTopic -> the keyword that already owns it } across claimed/done rows.
// Feeds the gate's topic-collision block (ctx.topicGuard.taken).
//
// Ownership is keyed on the CLAIMED ROW, not on article_slug: markClaimed() is
// called without an articleSlug everywhere in the pipeline (ffx-cron:345,
// keyword-run:283), so article_slug is always null and keying on it would make
// this guard silently inert. `excludeKeyword` drops the row for the job being
// gated right now — the cron marks a target claimed BEFORE enqueueing it, so
// without that exclusion every article would collide with its own claim.
export function takenTopics(map, excludeKeyword) {
  const out = {};
  const skip = String(excludeKeyword || '').toLowerCase().trim();
  for (const r of (Array.isArray(map) ? map : [])) {
    if (!r || (r.status !== 'claimed' && r.status !== 'done')) continue;
    if (skip && String(r.keyword || '').toLowerCase().trim() === skip) continue;
    const t = rowTopic(r);
    if (t && !out[t]) out[t] = r.article_slug || r.keyword;
  }
  return out;
}

// Select the next N winnable, unclaimed, distinct-HEAD-topic targets.
...
export function selectTargets(map, n) {
  ...
  const usedTopics = new Set(
    ...
    map.filter(r => r && (r.canonical || r.keyword) && (r.status === 'claimed' || r.status === 'done'))
       .map(rowTopic)
       .filter(Boolean)
  );
  for (const row of openWinnable) {
    ...
    if (!topic || usedTopics.has(topic)) continue; // one article per head topic
  ...
  for (const row of openWinnable) {
    const topic = rowTopic(row);
    if (topic && !usedTopics.has(topic)) remainingTopics.add(topic);
  }
