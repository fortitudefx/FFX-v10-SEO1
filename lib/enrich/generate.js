...
export async function generateSections(article, targets, nuggets, apiKey, fetchImpl = fetch) {
  ...
  // Blockquotes are forbidden in the prompt, but instruction-following is not a
  // guarantee and one page still emitted one, failing the whole batch on [quotes].
  // Demote rather than delete: the text is the model's paraphrase of a nugget, so
  // presenting it as ordinary prose is MORE accurate than attributing it to Salman
  // as something he said — and the content is kept.
  html = html.replace(/<blockquote[^>]*>/gi, '<p>').replace(/<\/blockquote>/gi, '</p>');
