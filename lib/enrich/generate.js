...
export function buildEnrichPrompt(article, targets, nuggets) {
  ...
  if (nuggets && nuggets.length) {
    L.push("SALMAN'S OWN WORDS — build on these. Any <blockquote> must reproduce one");
    L.push('VERBATIM, exactly as written. Never paraphrase inside a quote, never invent one.');
    L.push('');
    ...
    L.push("Salman's first-person voice, and use NO <blockquote> tags — there is nothing");
    L.push('to quote, so quoting would be fabrication.');
    L.push('');
  }
  L.push('HARD RULES');
