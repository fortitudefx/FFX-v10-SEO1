...
import { htmlToStructuralText, countWords } from './html.js';
...
export async function runGate(article, ctx = {}, env = {}) {
  ...
  const v = { status: 'passed', reason: null, similarity: null, structural: null, voice: null, bannedOpenings: null, quotes: null, fabrication: null, wordCount: null, at: new Date().toISOString() };
  const done = extra => ({ ...v, ...extra, at: new Date().toISOString() });

  // Word count is recorded for VISIBILITY ONLY — the gate never fails on length.
  // Google does not reward length for its own sake; a complete short article is a
  // pass. Substance is enforced by the voice/trust/fabrication checks, not a floor.
