...
export async function runGate(article, ctx = {}, env = {}) {
  ...
  // ── BLOCK 0: TOPIC COLLISION (intent-level cannibalization) ─────────────────
  // The similarity block below compares PROSE. Cannibalization is caused by shared
  // SERP INTENT, which prose similarity does not detect: "How to Trade Fair Value
  // Gap" and "How to Trade Fair Value Gaps" went live on the same day sharing only
  // ~10% of their wording — far under similarityMax — yet they target one query and
  // split each other's signal. Selection now dedupes by head topic, so this is the
  // second line of defence for jobs that bypass the demand map (manual runs,
  // re-queues, keyword-run). Cheapest check in the gate, so it runs first.
  if (ctx.topicGuard && ctx.topicGuard.topic) {
    v.topic = ctx.topicGuard.topic;
    const owner = (ctx.topicGuard.taken || {})[ctx.topicGuard.topic];
    if (owner && owner !== article.slug) {
      return done({ status: 'failed', reason: `[topic] "${ctx.topicGuard.topic}" is already covered by "${owner}" — intent-level duplicate (cannibalization), not a wording problem` });
    }
  }
  ...
  // ctx.addedBody: the text THIS run is responsible for. Judge duplication on it
  // Used by enrichment, where the whole body is the wrong unit. Adding sections to
  // an existing page makes it MORE comprehensive on its topic, which necessarily
  // raises its overlap with sibling pages covering the same concept — the enriched
  // liquidity-sweep page hit 0.57 against an article that merely carries a
  // "liquidity sweep" tag. That is comprehensiveness, not duplication. The real
  // question for enrichment is whether the NEW sections duplicate existing content,
  // so enrichment passes just those. Everything else (thin, voice, quotes,
  // fabrication) is still judged on the full merged body, where it belongs.
  // Absent the override this is byte-identical to previous behaviour.
  try {
    const vec = termVector(ctx.addedBody || body);
  ...
  // Uses ctx.addedBody when present, for the same reason similarity does: an
  // enrichment run is responsible for the text it ADDS, not for the article's
  // existing quotes. Those were verified against the nugget set current when the
  // article was written; re-checking them against the handful of nuggets matched
  // for today's topic flagged legitimate, already-published quotes as fabricated
  // and blocked 6 of 10 pages that had added no quotes at all.
  if (Array.isArray(ctx.nuggetTexts) && ctx.nuggetTexts.length) {
    const q = verifyQuotes(ctx.addedBody || body, ctx.nuggetTexts);
