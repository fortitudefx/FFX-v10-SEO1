...
- [ ] **✅** **[EEAT-yt]** **Surface the source YouTube video on each article page** — when `a.youtubeUrl` exists, `article.js` renders a "Watch the original video on YouTube" link (a link, not an embed — page stays light) below the body, opens in a new tab, styled like `.article-sibling`; graceful fallback (no url → nothing rendered, no error). Verified live: present w/ correct href where a url exists, absent cleanly where it doesn't; guard 33/33 incl. render check. *(6419b27)* **(done — preview)**
- [ ] **✅** **[EEAT-byline]** **Visible author byline on articles → `/about`** — `article.js` renders "By Salman Khan" (→ /about) under the title, understated/matching typography; Article JSON-LD `author.url` now points to `/about` (was homepage). Verified live on articles; guard 33/33. *(6419b27)* **(done — preview)**
